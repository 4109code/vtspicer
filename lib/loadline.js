...
export function resolveStageLoad({
  ...
  if (kind === 'output') {
    ...
    return { purpose: kind, topology: 'plate', rdc, rac, eta: eff, zLoad: rac };
  }
  if (kind === 'headphone') {
    const phones = zhp > 0 ? zhp : 0;
    const rac = phones > 0 ? parallelOhms(plate, phones) : plate;
    return { purpose: kind, topology: 'plate', rdc: plate, rac, eta: 1, zLoad: phones > 0 ? phones : rac };
  }
  const next = rg > 0 ? rg : 0;
  const rac = next > 0 ? parallelOhms(plate, next) : plate;
  return { purpose: 'preamp', topology: 'plate', rdc: plate, rac, eta: 1, zLoad: rac };
}
