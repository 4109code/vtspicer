/**
 * Shared tube sampling / clamp / inverse-fit, model-aware.
 */

import { getModel } from './models/registry.js';
import { parseVgList } from './models/math.js';

export { softplus, parseVgList } from './models/math.js';
export { listModels, getModel, defaultParams, modelSupports } from './models/registry.js';
...
export function screenCurrent(modelId, type, Eg, Ep, Eg2, params) {
  ...
  if (typeof m.screenCurrent !== 'function') return 0;
  return m.screenCurrent(type, Eg, Ep, Eg2, params);
}
...
export function curveFamily(modelId, type, params, opts) {
  ...
  for (const vg of vgList) {
    ...
    for (let i = 0; i < vpSteps; i++) {
      ...
      const ip = plateCurrent(modelId, type, vg, vp, eg2, params);
      points.push({ vp, ip });
    }
...
/** Screen-grid current family (same Vp sweep / Vg list as plate). Empty for triode. */
export function screenCurveFamily(modelId, type, params, opts) {
  ...
  for (const vg of vgList) {
    ...
    for (let i = 0; i < vpSteps; i++) {
      const vp = vpMin + i * dvs;
      const ig2 = screenCurrent(modelId, type, vg, vp, eg2, params);
      points.push({ vp, ip: ig2 });
    }
    curves.push({ vg, points });
  }
  return curves;
...
/**
 * Finite-difference Jacobian step toward targetIp.
 */
export function inverseFitStep(
  modelId,
  type,
  params,
  Eg,
  ...
  const model = getModel(modelId);
  const keys = activeKeys.filter((k) => params[k] != null && model.limits[k]);
  if (!keys.length) return { ...params };

  const ip0 = plateCurrent(modelId, type, Eg, Ep, Eg2, params);
  const residual = targetIp - ip0;
  if (Math.abs(residual) < 1e-12) return { ...params };

  const epsRel = 1e-4;
  const J = [];
  for (const key of keys) {
    const p = { ...params };
    const h = Math.max(Math.abs(params[key]) * epsRel, 1e-9);
    p[key] = params[key] + h;
    const ip1 = plateCurrent(modelId, type, Eg, Ep, Eg2, p);
    J.push((ip1 - ip0) / h);
  }

  let denom = 0;
  for (const j of J) denom += j * j;
  if (denom < 1e-30) return { ...params };

  const scale = (damping * residual) / denom;
  const next = { ...params };
  for (let i = 0; i < keys.length; i++) {
    next[keys[i]] = params[keys[i]] + scale * J[i];
  }
  return clampParams(modelId, next);
}
...
// Re-export Koren-specific helpers for older tests / direct imports
export {
  triodeIp as korenTriodeIp,
  pentodeIp as korenPentodeIp,
  screenIg2,
