...
/** KB is the plate/screen split; MU2 changes the shared cathode current. */
...
+ KB=${fmt(p.KB)} VCT=${fmt(p.VCT ?? 0)} ${pass.rgi}
+ ${pass.caps}
...
+ PARAMS: MU=${fmt(p.MU)} XS=${fmt(p.XS)} KS=${fmt(p.KS)} KP=${fmt(p.KP)}
+ KB=${fmt(p.KB)} MU2=${fmt(p.MU2 ?? 20)} VCT=${fmt(p.VCT ?? 0)} ${pass.rgi}
+ ${pass.caps}
EGG 10 0 VALUE={V(2,3)+VCT}
