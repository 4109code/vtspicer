...
function screenAtPoint(vp, vgk, { eg2, ul, vq, screenDrop }) {
  if (screenDrop?.rg2 > 0) {
...
export function intersectLoadLine(ipAt, vg, opts) {
  ...
  const miss = (vp) => {
    ...
    const screen = screenAtPoint(vp, vgk, { eg2, ul, vq, screenDrop: opts.screenDrop });
    return finiteIp(ipAt, vgk, vp, screen) - ik;
  };
  let lo = vpLo;
...
export function swingSamples(ipAt, { vg, vin, vq, iq, rp, eg2, ul, vpLo, vpHi, abortOnMiss = false, rk = 0, bypassed = true, screenDrop = null }) {
  const span = Number.isFinite(vin) ? vin : 0;
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const screen = vp == null ? eg2 : screenAtPoint(vp, vgk, { eg2, ul, vq, screenDrop });
...
export function analyzeLoadLine({
  ...
  const screenDrop = rg2 > 0 && ig2At && kind === 'preamp'
    ? { ig2At, vc: dropVc, rg2 }
    : null;
  const ss = smallSignal({
    ipAt: (g, p) => ipAt(g, p, screenDrop ? screenQ : screenVoltage(p, eg2, ul, vp)),
    vg,
    vp,
    eg2: screenQ,
    rp: kind === 'output' ? 0 : dc,
