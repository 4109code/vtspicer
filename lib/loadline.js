...
/** Quiescent screen and supply when the screen resistor hangs off the plate supply */
export function coupledScreen(ipAt, ig2At, { vg, vp, rdc = 0, rg2 = 0, eg2 = 0 }) {
  const dc = rdc > 0 ? rdc : 0;
  let screen = Number.isFinite(eg2) ? eg2 : 0;
  let vc = vp > 0 ? vp : 0;
  if (!(rg2 > 0) || typeof ig2At !== 'function' || !(vp > 0)) {
    const ip = Math.max(0, finiteIp(ipAt, vg, vp, screen));
    return { screen, vc: vp + dc * ip };
  }
  for (let n = 0; n < 48; n++) {
    const ip = Math.max(0, finiteIp(ipAt, vg, vp, screen));
    vc = vp + dc * ip;
    const next = droppedScreen(ig2At, vg, vp, vc, rg2);
    if (next == null) break;
    if (Math.abs(next - screen) < 1e-3) {
      screen = next;
      vc = vp + dc * Math.max(0, finiteIp(ipAt, vg, vp, screen));
      break;
    }
    screen = next;
  }
...
/** Small-signal voltage gain of the same load-line solve the swing uses */
function probedGain(ipAt, swing) {
  const probe = Math.max(0.005, Math.min(0.05, Math.abs(swing.vg) * 0.02 || 0.02));
  const row = swingSamples(ipAt, { ...swing, vin: probe });
  const lo = row[0];
  const hi = row[6];
  if (lo?.vp == null || hi?.vp == null) return null;
  const gain = Math.abs(hi.vp - lo.vp) / (2 * probe);
...
/** Vg that sits on a cathode resistor at this plate voltage: Ip(Vg) = −Vg / Rk */
export function vgForCathodeResistor(ipAt, vp, rk, eg2, { vgLo = VG_SEARCH_LO, vgHi = -0.001 } = {}) {
  if (!(rk > 0) || !(vp > 0) || typeof ipAt !== 'function') return null;
  const miss = (vg) => finiteIp(ipAt, vg, vp, eg2) + vg / rk;
  let lo = vgLo;
  let hi = vgHi;
  let flo = miss(lo);
  let fhi = miss(hi);
  ...
  for (let n = 0; n < 48; n++) {
    const mid = (lo + hi) / 2;
    const f = miss(mid);
    if (Math.abs(f) < 1e-9 || Math.abs(hi - lo) < 1e-6) return mid;
    if (flo * f <= 0) {
      hi = mid;
      fhi = f;
    } else {
...
export function analyzeLoadLine({
  ...
  let dropVc = vp;
  const dropping = rg2 > 0 && ig2At && kind === 'preamp' && !strap;
  if (dropping) {
    const solved = coupledScreen(ipAt, ig2At, { vg, vp, rdc: dc, rg2, eg2 });
    screenQ = solved.screen;
    dropVc = solved.vc;
  }
  const screenDrop = dropping ? { ig2At, vc: dropVc, rg2 } : null;
  if (strap) screenQ = vp;
  const ss = smallSignal({
  ...
  const levels = swingLevels(vin, vpp, ipp);
  let av = useRk > 0 ? degeneratedGain(ss.mu, ss.ra, ac, useRk) : stageGain(ss.mu, ss.ra, ac);
  if (screenDrop) av = probedGain(ipAt, swing) ?? av;
...
export function clipDrive({
  ...
  const dropping = rg2 > 0 && ig2At && purpose !== 'output' && purpose !== 'headphone' && !strap;
  const screenDrop = dropping ? { ig2At, vc: screenVc > 0 ? screenVc : vp, rg2 } : null;
  ...
  const at = (vin) => {
    ...
    return swingSamples(ipAt, {
      vg, vin, vq: vp, iq, rp: rac, eg2, ul, vpHi, rk, bypassed, screenDrop, strap,
    });
  };
  let hit = null;
  const steps = 28;
...
function candidatePoint(ipAt, ig2At, spec) {
  ...
  if (loads.kind === 'preamp' && spec.voutTarget > 0) {
    const full = finiteIp(ipAt, 0, vp, screenQ);
    if (full > 1e-6 && iq < full * 0.1) return null;
    if (rp > 470e3) return null;
  }
  ...
  const gridMax = Number.isFinite(spec.vgMax) ? spec.vgMax : 0;
  if (samples.some((s) => s.vg > gridMax + 1e-6)) return null;
...
/** Grid voltage a fraction of the way from vgMax toward cutoff */
function gridFromCeiling(vgMax, cut, frac) {
  return vgMax + (cut - vgMax) * frac;
}

/**
 * Q-point, load, and drive. With thdMax, the most output power at or below that THD.
 ...
 * vgMax caps grid voltage on the swing. 0 keeps it at or below zero.
 */
function optimizeFollower({ ipAt, ig2At, eg2, ul, pmax, thdMax, vcMax, vpHi, vpMin, zhp, pg2Max, vgMax = 0 }) {
  const gridMax = Number.isFinite(vgMax) ? vgMax : 0;
  const plotHi = Math.max(vpHi, 1);
  const mid = plotHi * 0.55;
  const cut = cutoffVg(ipAt, mid, eg2, ul);
  const vgValues = linspace(
    Math.min(gridMax - 0.4, gridFromCeiling(gridMax, cut, 0.85)),
    Math.min(gridMax - 0.3, gridFromCeiling(gridMax, cut, 0.2)),
    5,
  );
  ...
  let best = null;
  for (const vg of vgValues) {
    const headroom = gridMax - vg;
    if (!(headroom > 0.05)) continue;
    for (const vp of vpValues) {
      for (const drive of drives) {
        const op = analyzeFollower({
          ipAt, ig2At, vg, vp, zhp, vin: headroom * drive, eg2, ul, withSweep: false,
        });
        ...
        if (op.samples.some((s) => s.vg != null && s.vg > gridMax + 1e-6)) continue;
        if (!best || op.pout > best.pout) {
          best = {
            vg, vp, rp: op.rk, vin: headroom * drive, iq: op.ip, vc: op.vc,
            plateDissipation: op.plateDissipation,
            screenDissipation: op.screenDissipation,
            pout: op.pout, thd: op.thd, score: op.pout,
            voutRms: swingLevels(headroom * drive, op.vpp, op.ipp).voutRms,
          };
        }
      }
    }
...
export function optimizeLoadLine({
  ...
  if (purpose === 'headphone' && topology === 'follower') {
    return optimizeFollower({
      ipAt, ig2At, eg2, ul, pmax, thdMax, vcMax, vpHi, vpMin, zhp, pg2Max, vgMax: gridMax,
    });
  }
  const plotHi = Math.max(vpHi, 1);
  const mid = plotHi * 0.55;
  const cut = cutoffVg(ipAt, mid, eg2, ul);
  const vgHi = Math.min(gridMax - 0.2, gridFromCeiling(gridMax, cut, 0.12));
  const vgLo = Math.min(vgHi - 0.3, gridFromCeiling(gridMax, cut, 0.92));
  const vpLo = Math.max(plotHi * (voutTarget > 0 && purpose === 'preamp' ? 0.3 : 0.12), vpMin);
  ...
  const fine = searchBox(
    ...
    ig2At,
    {
      ...
      driveLo: Math.max(0.3, coarse.vin / (gridMax - coarse.vg) - 0.18),
      driveHi: Math.min(1, coarse.vin / (gridMax - coarse.vg) + 0.12),
    },
