...
import { pentodeDrive, spaceCharge, PENTODE_E1, PENTODE_PLATE } from './koren.js';
import {
  fmt,
  fmtPassives,
  PASSIVES,
...
+ KVB=${fmt(p.KVB)} VCT=${fmt(p.VCT ?? 0)} ${pass.rgi}
+ ${pass.caps}
RE1 7 0 1MEG
${PENTODE_E1}
G1 1 3 VALUE={${PENTODE_PLATE}/KG1*${knee}}
G2 4 3 VALUE={URAMP(PWR(URAMP(V(4,3)/MU+V(2,3)+VCT),EX)/KC-${PENTODE_PLATE}/KG1*${knee})}
RCP 1 3 1G
${spiceTail('pentode')}`;
}
