...
export function screenIg2(Eg, Eg2, params, Ep = 0) {
  ...
  return cutoffPower(Eg, Eg2, MU, EX, KG2, VCT);
}
...
export function pentodeE1(pins) {
  const n = resolvePins('pentode', pins);
  return `E1 7 0 VALUE={V(${n.G2},${n.C})/KP*LOG(1+EXP(KP*(1/MU+(V(${n.G},${n.C})+VCT)/V(${n.G2},${n.C}))))}`;
}
...
export function generateSubckt({ name = 'TUBE', type = 'triode', params, comment = '', pins } = {}) {
  ...
  const strapped =
    gurskii && p.KG1 > 0 && p.KG2 > 0
      ...
      : '';
  const tail = child
    ? spiceTail(type, { gridDiode: false, pins }).replace('.ENDS\n', `${childLawSources(pins, type)}\n.ENDS\n`)
    : spiceTail(type, { pins });

  if (type === 'triode') {
    return `${note}.SUBCKT ${safeName} ${n.list.join(' ')} ; P G C (Triode / Koren)
+ PARAMS: MU=${fmt(p.MU)} EX=${fmt(p.EX)} KG1=${fmt(p.KG1)} KP=${fmt(p.KP)}
+ KVB=${fmt(p.KVB)} VCT=${fmt(p.VCT ?? 0)} ${gridSpec}
+ ${pass.caps}
E1 7 0 VALUE={V(${n.P},${n.C})/KP*LOG(1+EXP(KP*(1/MU+(V(${n.G},${n.C})+VCT)/SQRT(KVB+V(${n.P},${n.C})*V(${n.P},${n.C})))))}
RE1 7 0 1G
G1 ${n.P} ${n.C} VALUE={${PENTODE_PLATE}/KG1}
RCP ${n.P} ${n.C} 1G
${tail}`;
  }

  return `${note}.SUBCKT ${safeName} ${n.list.join(' ')} ; P G1 C G2 (Pentode/tetrode / Koren)
...
${pentodeE1(pins)}
G1 ${n.P} ${n.C} VALUE={${PENTODE_PLATE}/KG1*ATAN(V(${n.P},${n.C})/KVB)}
G2 ${n.G2} ${n.C} VALUE={${
    gurskii
      ? `V(7)/KG2*(KVC-ATAN(V(${n.P},${n.C})/KVB))`
      : `(EXP(EX*(LOG((V(${n.G2},${n.C})/MU)+V(${n.G},${n.C})))))/KG2`
  }}
RCP ${n.P} ${n.C} 1G
${tail}`;
}
