...
export const paramKeys = [
  'MU', 'EX', 'KG', 'KP', 'KN', 'KS', 'BK', 'RS', 'MU2', 'VCT', 'ND', 'DH',
  'DD1', 'VD1', 'WD1',
  'DD2', 'VD2', 'WD2',
  'DD3', 'VD3', 'WD3',
];
export const paramHints = {
  ...
  KN: {
    ...
    pentode: 'Knee voltage. Higher moves the bend right',
  },
  KS: 'Knee sharpness. 1 is the soft bend. Higher squares it off',
  BK: 'Knee shift. Higher moves the bend right as Vg goes more negative',
...
function appendDipKeys(base, params) {
  const keys = [...base];
...
export function fitKeyList(type, params) {
  return appendDipKeys(inverseKeys[type] || inverseKeys.triode, params);
}

export const inverseKeys = {
  triode: ['MU', 'EX', 'KG', 'KP', 'KN'],
  pentode: ['MU', 'EX', 'KG', 'KP', 'KN', 'KS', 'BK', 'RS', 'MU2', 'VCT'],
};

/** RS is the high-Vp screen share. MU2 changes the shared cathode current. */
export const screenKeys = {
  pentode: ['RS', 'MU2', 'KS', 'BK'],
};

export const multiGridParams = screenKeys.pentode;

export function screenKeyList(type, params) {
  const base = screenKeys[type] || [];
  if (!base.length) return [];
  return appendDipKeys(base, params);
}
...
/** 1 is ep/(ep+KN). Higher KS squares the knee. BK shifts it right as drive falls. */
function kneeShare(ep, params, u, u0) {
  ...
  const bk = params.BK == null || !Number.isFinite(Number(params.BK)) ? 0 : Number(params.BK);
  const grow = bk === 0 ? 1 : shiftedRatio(u0, u, bk);
  const kne = kn * grow;
  if (!(ep > 0)) return 0;
  if (ks === 1) return ep / (ep + kne);
  const epk = ep ** ks;
  const denom = epk + kne ** ks;
  return denom > 0 ? epk / denom : 0;
}

function plateFraction(ep, params, taper, u, u0) {
  const rs = Math.min(0.95, Math.max(0, params.RS ?? 0.14));
  return (1 - rs) * kneeShare(ep, params, u, u0) * notch(ep, params, taper);
}
...
function driveAt(Eg, va, params) {
  ...
  return { u, u0, taper: currentTaper(u, u0, params.EX, highDip(params)) };
}
...
function pentodeCurrents(Eg, Ep, Eg2, params) {
  ...
  const va = eg2 + ep / mu2;
  const { u, u0, taper } = driveAt(Eg, va, params);
  const ik = cathodeFromU(u, params);
  const fp = plateFraction(ep, params, taper, u, u0);
...
function spiceSoftplus(expr) {
  return `(MAX(${expr},0)+LOG(1+EXP(-ABS(${expr}))))`;
}

function ridgeDrive(va, n) {
  return `EGG 10 0 VALUE={V(${n.G},${n.C})+VCT}
EVA 11 0 VALUE={${va}}
EL 12 0 VALUE={V(10)+V(11)/MU}
EA 13 0 VALUE={KP*V(12)}
ES 14 0 VALUE={${spiceSoftplus('V(13)')}/KP}
EIK 15 0 VALUE={PWR(URAMP(V(14)),EX)/KG}
EU 17 0 VALUE={${spiceSoftplus('KP*(VCT+V(11)/MU)')}/KP}`;
}

export function generateSubckt({ name = 'TUBE', type = 'triode', params, comment = '', pins } = {}) {
  ...
  if (type === 'triode') {
    return `${note}.SUBCKT ${safeName} ${n.list.join(' ')} ; P G C (Triode / Ridge)
+ PARAMS: MU=${fmt(p.MU)} EX=${fmt(p.EX)} KG=${fmt(p.KG)} KP=${fmt(p.KP)}
+ KN=${fmt(p.KN)} VCT=${fmt(p.VCT ?? 0)} ${pass.rgi}
+ ${pass.caps}${dips}
${ridgeDrive(`URAMP(${vp})`, n)}
G1 ${n.P} ${n.C} VALUE={V(15)*V(11)/(V(11)+KN)${spiceNotch('V(11)', p)}+1e-12*${vp}}
${spiceTail('triode', { pins })}`;
  }

  const vg2 = `V(${n.G2},${n.C})`;
  return `${note}.SUBCKT ${safeName} ${n.list.join(' ')} ; P G1 C G2 (Pentode/tetrode / Ridge)
+ PARAMS: MU=${fmt(p.MU)} EX=${fmt(p.EX)} KG=${fmt(p.KG)} KP=${fmt(p.KP)}
+ KN=${fmt(p.KN)} KS=${fmt(p.KS ?? 1)} BK=${fmt(p.BK ?? 0)} RS=${fmt(p.RS ?? 0.14)} MU2=${fmt(p.MU2 ?? 22)} VCT=${fmt(p.VCT ?? 0)}
+ ${pass.rgi}
+ ${pass.caps}${dips}
${ridgeDrive(`URAMP(${vg2})+URAMP(${vp})/MU2`, n)}
EKN 18 0 VALUE={KN*PWR(MAX(V(17),1E-9)/MAX(V(14),1E-6),BK)}
EFP 16 0 VALUE={(1-RS)*PWR(URAMP(${vp}),KS)/(PWR(URAMP(${vp}),KS)+PWR(V(18),KS))${spiceNotch(`URAMP(${vp})`, p)}}
G1 ${n.P} ${n.C} VALUE={V(15)*V(16)+1e-12*${vp}}
G2 ${n.G2} ${n.C} VALUE={V(15)*(1-V(16))+1e-12*${vg2}}
