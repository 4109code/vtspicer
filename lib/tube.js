...
export function peakOf(targets) {
  let peak = 0;
...
function peakGuard(modelId, type, params, targets) {
  const peakSamples = familyPeakSamples(targets);
  return {
    peakSamples,
    startPeak: familyPeak(modelId, type, params, peakSamples),
    maxTarget: peakOf(targets),
  };
}
...
function fitWithDips(modelId, type, params, targets, fitKeys, opts) {
  const { iterations = 80 } = opts;
  const model = getModel(modelId);
  const shared = peakGuard(modelId, type, params, targets);
  ...
  const runDip = (start, dipIters) => {
    ...
    return solveToConvergence(modelId, type, start, targets, keys, {
      ...shared,
      iterations: dipIters,
      project,
      scaleOf,
  ...
  const errSeed = sumSquaredError(modelId, type, fromSeed, targets);
  const errHere = sumSquaredError(modelId, type, fromHere, targets);
  const placed = errSeed < errHere ? fromSeed : fromHere;

  let p = solveToConvergence(modelId, type, placed, targets, smoothKeys, {
    ...shared,
    iterations,
  });
  p = runDip(p, 24);
  p = solveToConvergence(modelId, type, p, targets, smoothKeys, {
    ...shared,
...
export function fitToTargets(modelId, type, params, targets, opts = {}) {
  const { iterations = 80, keys = null } = opts;
  ...
  if (hasDips && typeof model.clampDipFit === 'function') {
    return fitWithDips(modelId, type, seeded, targets, fitKeys, { iterations });
  }
  ...
  if (model.enumParams?.KNEE && !fitKeys.includes('KNEE')) {
    ...
    for (const knee of [0, 1]) {
      ...
      const next = solveToConvergence(modelId, type, start, targets, fitKeys, {
        iterations,
        ...peakGuard(modelId, type, start, targets),
      });
      const err = sumSquaredError(modelId, type, next, targets);
      if (err < bestErr) {
        best = next;
        bestErr = err;
      }
  ...
  return solveToConvergence(modelId, type, seeded, targets, fitKeys, {
    iterations,
    ...peakGuard(modelId, type, seeded, targets),
  });
}
