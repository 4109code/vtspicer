...
+ RGI=${fmtOhm(p.RGI ?? pass.RGI)} CCG=${fmtCap(p.CCG ?? pass.CCG)} CGP=${fmtCap(p.CGP ?? pass.CGP)} CCP=${fmtCap(p.CCP ?? pass.CCP)}
.PARAM A={(3-3*ALPHA)/2}
...
+ RGI=${fmtOhm(p.RGI ?? pass.RGI)} CCG=${fmtCap(p.CCG ?? pass.CCG)} CPG1=${fmtCap(p.CGP ?? pass.CGP)} CCP=${fmtCap(p.CCP ?? pass.CCP)}
.PARAM A={(3-3*ALPHA)/2}
.PARAM EXPA={1/(1-ALPHA)}
.PARAM EXPB={1.5-1/(1-ALPHA)}
.PARAM DEL={1/MUC-1/MUM}
