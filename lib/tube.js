...
/**
 * Column-scaled Levenberg–Marquardt. `project` runs after the limit clamp.
...
function levenberg(modelId, type, params, rows, fitKeys, opts) {
  ...
  const baseScales = parameterScales(model, fitKeys, p);
  const custom = scaleOf ? scaleOf(fitKeys, p) : null;
  const scales = baseScales.map((s, i) => (custom?.[i] > 0 ? custom[i] : s));
  let lambda = 0.1;
  ...
  for (let iter = 0; iter < iterations; iter++) {
    ...
    const probes = fitKeys.map((key, i) => {
      ...
      return { h, trial: clampParams(modelId, { ...p, [key]: p[key] + h }), scale: scales[i] };
    });
    ...
    for (const t of rows) {
      ...
      const r = w * (t.ip - ip0);
      const J = probes.map(({ h, trial, scale }) => {
        const ip1 = targetCurrent(modelId, type, t, trial);
...
function fitWithDips(modelId, type, params, targets, fitKeys, opts) {
  ...
  const model = getModel(modelId);
  const rows = [...targets, ...anchorRows(modelId, type, params, targets)];
  ...
  const runDip = (start, dipIters) => {
    ...
    return levenberg(modelId, type, start, rows, keys, {
      ...shared,
      iterations: dipIters,
      project,
      scaleOf,
  ...
  let p = levenberg(modelId, type, placed, rows, smoothKeys, {
    ...shared,
    iterations,
  });
  p = runDip(p, 24);
  p = levenberg(modelId, type, p, rows, smoothKeys, {
    ...shared,
...
export function fitToTargets(modelId, type, params, targets, opts = {}) {
  ...
  const rows = [...targets, ...anchorRows(modelId, type, seeded, targets)];
  const peakSamples = familyPeakSamples(targets);
  return levenberg(modelId, type, seeded, rows, fitKeys, {
    iterations,
    damping,
    peakSamples,
