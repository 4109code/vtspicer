...
export const multiGridParams = ['RS', 'MU2', 'KS', 'BK'];
...
function kneeShare(ep, params, u, u0) {
  ...
  const grow = bk === 0 ? 1 : (Math.max(u0, 1e-9) / Math.max(u, 1e-6)) ** bk;
  const kne = kn * grow;
  if (!(ep > 0)) return 0;
...
ES 14 0 VALUE={(MAX(V(13),0)+LOG(1+EXP(-ABS(V(13)))))/KP}
EIK 15 0 VALUE={PWR(URAMP(V(14)),EX)/KG}
EU 17 0 VALUE={(MAX(KP*(VCT+V(11)/MU),0)+LOG(1+EXP(-ABS(KP*(VCT+V(11)/MU)))))/KP}`;
}
