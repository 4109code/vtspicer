...
export function kneeFactor(Ep, params, Eg = 0, Eg2 = 0) {
  ...
  let kvb = kvb0;
  if (kb > 0 && Eg2 > 0) {
    const e0 = Math.max(pentodeDrive(0, Eg2, params), 1e-9);
    const e = Math.max(pentodeDrive(Eg, Eg2, params), 1e-6);
    kvb = kvb0 * (e0 / e) ** kb;
  }
...
function kneeSpice(knee, n, kvb, sharp) {
  const vp = `V(${n.P},${n.C})`;
  if (!sharp && kvb === 'KVB') {
    if ((knee ?? 0) >= 0.5) return `1.57*TANH(2*${vp}/(KVB*3.14159))`;
    return `ATAN(${vp}/KVB)`;
  }
  const x = sharp ? `PWR(${vp}/${kvb},KS)` : `${vp}/${kvb}`;
  if ((knee ?? 0) >= 0.5) return `1.57*TANH((2/3.14159)*(${x}))`;
  return `ATAN(${x})`;
}
