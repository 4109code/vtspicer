...
export function resolveStageLoad({
  ...
  return { purpose: 'preamp', topology: 'plate', vertical: false, rdc: plate, rac, eta: 1, zLoad: rac };
}
...
export function intersectLoadLine(ipAt, vg, { vq, iq, rp, eg2, ul = 0, vpLo = 0, vpHi = 2000, rk = 0, vExt = null }) {
  const miss = (vp) => {
...
export function swingSamples(ipAt, { vg, vin, vq, iq, rp, eg2, ul, vpLo, vpHi, abortOnMiss = false, rk = 0, bypassed = true }) {
  const span = Number.isFinite(vin) ? vin : 0;
...
function candidatePoint(ipAt, ig2At, { vg, vp, rp, vin, eg2, ul, pmax, thdMax, vcMax, vpLo, vpHi, iq: knownIq }) {
  ...
  return { vg, vp, rp, vin, iq, vc, plateDissipation, screenDissipation, pout, thd, eta, pdc, score };
}
  ...
  for (const vg of vgValues) {
    ...
    for (const vp of vpValues) {
      ...
      for (const slope of slopeValues) {
        ...
        if (box.vcMax > 0 && vp + rp * iq > box.vcMax * 1.002) continue;
