...
const PASSIVES = ['RGI', 'CCG', 'CGP', 'CCP'];
...
const FAMILIES = [
  {
    id: 'ayumi',
    test: (keys) =>
      keys.has('MUC') || keys.has('MUM') || keys.has('GLIM') || keys.has('ALPHA'),
  },
  { id: 'karpov', test: (keys) => keys.has('KC') },
  { id: 'immler', test: (keys) => keys.has('XS') || keys.has('KS') },
...
function identifyModel(keys, comment) {
  ...
  if (hinted && listModels().some((model) => model.id === hinted)) {
    const family = FAMILIES.find((item) => item.id === hinted);
    if (!family || family.test(keys) || matched.length === 0) return hinted;
  }
...
export function parseSpiceImport(text) {
  ...
  const keep = new Set([...model.paramKeys, ...PASSIVES]);
  const params = {};
  ...
  return { ok: true, modelId, type, name, params };
}
