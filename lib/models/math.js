...
/** (max(hi, 1e-9) / max(lo, 1e-6)) ** exp */
export function shiftedRatio(hi, lo, exp) {
  return (Math.max(hi, 1e-9) / Math.max(lo, 1e-6)) ** exp;
}

export function cutoffPower(Eg, Eg2, MU, EX, scale, VCT = 0) {
  if (!(Eg2 > 0) || !(scale > 0) || !(MU > 0) || !(EX > 0)) return 0;
  const e = Eg + VCT + Eg2 / MU;
  if (e <= 0) return 0;
  return Math.pow(e, EX) / scale;
}
